...
static bool idaapi pdiff_menu_callback(void *ud)
{
	...
	const char format[] =
			...
			"<#Saves PatchDiff3 results into the current IDB#Save results to IDB :C>>\n\n"
			;
...
options_t * options_init()
{
	...
	add_menu_item("Options/", "PatchDiff3", NULL, SETMENU_APP, pdiff_menu_callback, opt);
...
{
	del_menu_item("Options/PatchDiff3");
	if (opt) qfree(opt);
}
